...
|---|---|---|
| `Connections` → `Create` → `MySQL` UI | `cyntex new --kind source --connector mysql ...` plus a workspace YAML artifact | Current TapState implementation exposes a CLI authoring workflow, not the TapData console. |
