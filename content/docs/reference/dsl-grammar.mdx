...
description: Complete syntax reference for the TapState DSL (.tapstate.yml) — multi-resource declaration model
sidebar:
  order: 1
---
...
The TapState DSL is YAML format with file extension `.tapstate.yml`, declared with `version: tapstate/v1`.
...
`tapstate validate <file.tapstate.yml>` runs three layers of validation:
