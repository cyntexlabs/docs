...
description: Documentation contract for TapState resource files, relationships, and validation rules
sidebar:
  order: 1
ai:
  kind: reference
...
The current documentation contract uses YAML files with the extension `.tapstate.yml` and `version: tapstate/v1`.

<Aside type="caution" title="Verify against the selected product artifact">
  The latest implementation and rebrand have not yet been established as this site's release baseline. Treat this page as the resource contract to reconcile, not proof that an artifact accepts every field. The site does not currently publish a stable JSON Schema URL.
</Aside>
...
The documented validation model compares connector and mode combinations with a bundled capability Catalog. Verify the exact rejection behavior against the selected artifact.
...
Additional transform designs are not part of this reference until an identified TapState artifact publishes their schema and behavior.
...
- Runtime evaluation depends on the selected implementation contract.
- Script nodes can express logic beyond the documented expression fields when the artifact exposes them.
...
3. **Capability layer**: connector and mode validity plus the Catalog rules implemented by the selected artifact

Verify command syntax, required fields, unknown-field behavior, exit codes, and diagnostic format against the artifact supplied by your deployment.
